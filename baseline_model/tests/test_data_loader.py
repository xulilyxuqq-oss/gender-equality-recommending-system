    ...
    def test_loads_only_target_and_potential_neighbors(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            _write_jsonl(
                users_path,
                [
                    {"user_id": "u1", "completed_course_ids": ["c1"]},
                    {"user_id": "u2"},
                    {"user_id": "unrelated"},
                ],
            )
            ...
            self.assertEqual(data.completed_courses["u1"], frozenset({"c1"}))

    def test_rejects_unknown_completed_course(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            users_path = root / "users.jsonl"
            courses_path = root / "courses.jsonl"
            ...
            _write_jsonl(
                users_path,
                [{"user_id": "u1", "completed_course_ids": ["missing"]}],
            )
            _write_jsonl(
                courses_path,
                [{"course_id": "c1", "course_name": "课程一"}],
            ...
            with self.assertRaisesRegex(ValueError, "未知完成课程"):
                load_baseline_data(
                    users_path,
                    courses_path,
                    interactions_path,
